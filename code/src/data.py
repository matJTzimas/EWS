...
class DirectoryDataset(Dataset):
    def __init__(self, root, path, image_set, transform, target_transform):
        ...
        if self.split == 'train':
            if f'{cfg.dir_dataset_name}_shifts.pt' in os.listdir('../saved_models')  :
                self.shifts = torch.load(f'../saved_models/{cfg.dir_dataset_name}_shifts.pt')
                print(f'{cfg.dir_dataset_name}_shifts.pt  Loaded succesfully')
            else : 
                print("Not available shifts found for this dataset. Initialize shift to 0.15\n \
                    Check precompute_shifts_gmm.py")
                self.shifts = torch.zeros((self.img_files.shape[0],28,28)) + 0.15
    ...
    def __getitem__(self, index):
        ...
        if self.split == 'train':
            self.iteration += 1
            # print(self.iteration,  len(os.listdir(self.img_dir)))

            # if self.iteration > 400 :
            #     self.shifts = 0.985 * self.shifts
            #     self.iteration = 0 
            #     print('UPDATE SHIFTS')
        ...
        if self.split == 'train' :
            img_shifts = self.shifts[index,:,:]
        else :
            img_shifts = torch.rand((28,28))
            
        return img, label, mask, img_shifts
...
class ContrastiveSegDataset(Dataset):
    def __init__(self,
        ...
        # print(f'contrastive function -> {pytorch_data_dir}')
